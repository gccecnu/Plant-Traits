...
# windowsFonts(Times=windowsFont("TT Times New Roman"))##?趨??ͼ????Ϊtime new roman
...
# OpenWindow(13,16)
# # subplot (2,2)
# op=par(mfrow=c(3,2),mar=c(3.7, 4.2, 2, 1.0) + 0.2,lwd=3, bty="n", las=0, cex.axis=1.5,cex=1.3,
#        cex.lab=1.5,mgp=c(2.5,0.8,0))
# par(mai=c(1.0,1.2,0.2,0.2))

# set color
# coul = brewer.pal(2, "Accent") #Accent,Dark2,Paired,Pastel1,Pastel1,Set1,Set2,Set3
...
# mydata$Species = factor(mydata$Species, levels = c("CD","CS","IC","SM","ST","P"),
...
# dat_1 = data[data$xDistance == "0"|data$xDistance == "5"|
#                data$xDistance == "10",]
# dat_2 = data[data$xDistance == "10"|data$xDistance == "15"|
#                data$xDistance == "20",]
# dat_3 = data[data$xDistance == "20"|data$xDistance == "25"|
#                data$xDistance == "30",]
# dat_1$xDistance = "0"
# dat_2$xDistance = "15"
# dat_3$xDistance = "30"
...
colnames(dat_Plot_ST) = c("Height_trans", "Diameter_trans", "Distance", "num")
dat_Plot_ST = na.omit(dat_Plot_ST)

ft1 = sma(Height_trans ~ Diameter_trans * Distance, log = "xy", shift = TRUE, data = dat_Plot_ST)
...
# 绘图
p1 = ggplot(dat_Plot_ST, aes(x = Diameter_trans, y = Height_trans, color = Distance)) +
  ...
  # theme_minimal() +
  annotate("text", x = .7, y = 10000, label = expression(italic("Scirpus triqueter")), size = 6, hjust = 0) +
  annotate("text", x = c(.7, .7, .7), y = c(5500, 3000, 1600), label = labels, color = cbPalette[1:3], size = 6, hjust = 0) +
  mytheme
p1
...
colnames(dat_Plot_SM) = c("Height_trans", "Diameter_trans", "Distance", "num")
dat_Plot_SM = na.omit(dat_Plot_SM)

# 拟合模型
ft1 <- sma(Height_trans ~ Diameter_trans * Distance, log = "xy", shift = TRUE, data = dat_Plot_SM)
a_values <- coef(ft1)[1:3,2]  # Slope
...
# 绘图
p2 = ggplot(dat_Plot_SM, aes(x = Diameter_trans, y = Height_trans, color = Distance)) +
  geom_point(alpha = 0.6, size = 2) +
  ...
  labs(x = 'Diameter (mm)', y = 'Height (mm)') +
  # theme_minimal() +
  annotate("text", x = .7, y = 10000, label = expression(italic("Scirpus mariqueter")), size = 6, hjust = 0) +
  annotate("text", x = c(.7, .7, .7), y = c(5500, 3000, 1600), label = labels, color = cbPalette[1:3], size = 6, hjust = 0) +
  mytheme
p2
...
colnames(dat_Plot_CD) = c("Height_trans", "Diameter_trans", "Distance", "num")
dat_Plot_CD = na.omit(dat_Plot_CD)

# 拟合模型
ft1 <- sma(Height_trans ~ Diameter_trans * Distance, log = "xy", shift = TRUE, data = dat_Plot_CD)
...
# 绘图
p3 = ggplot(dat_Plot_CD, aes(x = Diameter_trans, y = Height_trans, color = Distance)) +
  geom_point(alpha = 0.6, size = 2) +
  scale_x_log10(breaks = x_breaks, labels = x_labels, limit = x_limit) +
  scale_y_log10(breaks = y_breaks, labels = y_labels, limit = y_limits) +
  geom_segment(data = data.frame(range_data[1, ]),
  ...
  labs(x = 'Diameter (mm)', y = 'Height (mm)') +
  # theme_minimal() +
  annotate("text", x = .7, y = 10000, label = expression(italic("Carex scabrifolia")), size = 6, hjust = 0) +
  annotate("text", x = c(.7, .7, .7), y = c(5500, 3000, 1600), label = labels, color = cbPalette[1:3], size = 6, hjust = 0) +
  mytheme
p3
...
colnames(dat_Plot_CS) = c("Height_trans", "Diameter_trans", "Distance", "num")
dat_Plot_CS = na.omit(dat_Plot_CS)

# 拟合模型
ft1 <- sma(Height_trans ~ Diameter_trans * Distance, log = "xy", shift = TRUE, data = dat_Plot_CS)
a_values <- coef(ft1)[1:3,2]
b_values <- coef(ft1)[1:3,1]
labels <- paste("a =", formatC(a_values, format = "f", digits = 2), 
                ", b =", formatC(b_values, format = "f", digits = 2))
...
# 绘图
p4 = ggplot(dat_Plot_CS, aes(x = Diameter_trans, y = Height_trans, color = Distance)) +
  geom_point(alpha = 0.6, size = 2) +
  scale_x_log10(breaks = x_breaks, labels = x_labels, limit = x_limit) +
  scale_y_log10(breaks = y_breaks, labels = y_labels, limit = y_limits) +
  geom_segment(data = data.frame(range_data[1, ]),
  ...
  labs(x = 'Diameter (mm)', y = 'Height (mm)') +
  # theme_minimal() +
  annotate("text", x = .7, y = 10000, label = expression(italic("Juncellus serotinus")), size = 6, hjust = 0) +
  annotate("text", x = c(.7, .7, .7), y = c(5500, 3000, 1600), label = labels, color = cbPalette[1:3], size = 6, hjust = 0) +
  mytheme
p4
...
colnames(dat_Plot_IC) = c("Height_trans", "Diameter_trans", "Distance", "num")
dat_Plot_IC = na.omit(dat_Plot_IC)
# 拟合模型
ft1 <- sma(Height_trans ~ Diameter_trans * Distance, log = "xy", shift = TRUE, data = dat_Plot_IC)
...
# 绘图
p5 = ggplot(dat_Plot_IC, aes(x = Diameter_trans, y = Height_trans, color = Distance)) +
  geom_point(alpha = 0.6, size = 2) +
  ...
  labs(x = 'Diameter (mm)', y = 'Height (mm)') +
  # theme_minimal() +
  annotate("text", x = .7, y = 10000, label = expression(italic("Imperata cylindrica")), size = 6, hjust = 0) +
  annotate("text", x = c(.7, .7 ), y = c(5500, 3000), label = labels, color = cbPalette[1:2], size = 6, hjust = 0) +
  mytheme
p5
...
colnames(SP_dis) = c('Dis',"Scirpus mariqueter",  "Scirpus triqueter",
                     "Juncellus serotinus",  "Carex scabrifolia",  "Imperate cylindrica")
data_long <- pivot_longer(SP_dis, cols = -Dis, names_to = "Group", values_to = "Value")

# Get allometry exponent
SP = unique(data_long$Group)
k = 0
k = k[-1]

rsquared = 0
rsquared = rsquared[-1]
for (i in 1:length(unique(data_long$Group))){
  lf_temp = data_long %>% 
    filter(data_long$Group == SP[i])
...
p6 = ggplot(data_long, aes(x = Dis, y = Value, color = Group, shape = Group, group = Group)) +
  # geom_line(size = 1) +  # 绘制折线
  geom_point(size = 4, alpha = 0.6) +  # 添加数据点
  scale_shape_manual(values = shape_values) +
  scale_color_manual(values = cbPalette) +
  geom_smooth(method = "lm", se = FALSE) +  # 添加虚线拟合线
  labs(x = "Distance to creek (m)", y = "Exponent of allometry (a)",
  ...
  theme(
    ...
    legend.text = element_text(face = "italic",size = 14),  # 设置图例字体为斜体
...
for(i in c(4,5,3,1,2)){
  text1 = bquote(k == .(round(k[i], 2)))
  text2 = bquote(R^2 == .(round(rsquared[i], 2)))
  p6 = p6 +
    annotate("text", x = 34, y = 2.1 - .2*(j), label = text1,
             size = 5, color = cbPalette[j], hjust = 0) +
